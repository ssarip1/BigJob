############################################
#  Centralized Asynchronous RE-Exchange       
#############################################
...
class ReManager():
   ...
   def start_bigjob(self,COORDINATION_URL,RESMGR_URL,i):
       ...
       #####################################################
       try:
          #print "\n (Delete Remote Directory If Exists)" + "ssh "+ str(RESMGR_URL[10:])+ " rm -rf "+ self.working_directory+"async_agent_8"
          os.system(" ssh " + str(RESMGR_URL[10:]) + " rm -rf "+ self.working_directory + "async_agent_8")
          print "\n (Success) Deleted Existing BigJobAgent Working Directory"
          os.system(" ssh " + str(RESMGR_URL[10:]) + " mkdir "+ self.working_directory + "async_agent_8")
       ...
       number_of_processes=32  # The total number of processes ( BigJob size), used to run Jobs
   ...
   ################################################################################################################
   # Stage all the Namd configuration files from local replica working directory to each replica working directory
   ################################################################################################################

   def stage_in_files(self,replica_id,RESMGR_URL):
       #start = time.time()
       #pdb.set_trace()
       #print "\n (INFO) " + "scp -r " + self.replica_directory + "* " + str(RESMGR_URL)+ ":"+self.working_directory + "async_agent_8/" + str(replica_id) + "/"
       #print "\n (INFO) " + "scp -r " + self.working_directory + "async_agent_8/ " + str(replica_id) + " " + str(RESMGR_URL)+ ":"+self.working_directory + "async_agent/"
       i=replica_id
       """try:
           os.mkdir(self.working_directory + "async_agent_8/" + str(replica_id))
       except:
       ...
       if(i<self.RPB):
          try:
             os.mkdir(self.working_directory + "async_agent_8/" + str(replica_id))
             os.system("cp -r " + self.replica_directory + "* " + self.working_directory + "async_agent_8/" + str(replica_id) + "/")
          ...
          try:
             os.system(" ssh " + str(RESMGR_URL) + " mkdir "+ self.working_directory + "async_agent_8/"+ str(replica_id))
             os.system("scp -r " + self.replica_directory + "* " + str(RESMGR_URL)+ ":"+self.working_directory + "async_agent_8/" + str(replica_id) + "/")
             #print "\n (INFO) total time taken to stage files on " + str(RESMGR_URL) + "is : "+ str(time.time()-start)
          except:
             print "\n (INFO) Error" + str(RESMGR_URL) 
   ...
   ################################################################################################
   # For Each Replica, set the Temperature value from default value to temperature you want to set
   ################################################################################################ 
   ...
   def transfer_NPT(self, replica_id,RESOURCEMGR_URL):
       ...
       #print "\n (INFO) " + "scp " + self.working_directory + "NPT.conf " + str(RESOURCEMGR_URL) + ":" +self.working_directory + "async_agent_8/" + str(replica_id) + "/"
       try:
          os.system("scp " + self.working_directory + "NPT.conf " + str(RESOURCEMGR_URL) + ":" +self.working_directory + "async_agent_8/" + str(replica_id) + "/")
          #print "\n (INFO) total time taken to transfer NPT on " + str(RESOURCEMGR_URL)+ " is: " + str(time.time()-start)
       except OSError:
          print "Unexpected error:", sys.exc_info() [0]

   #####################################################
   # Get the job description for each Replica
   ######################################################
   def get_job_description(self, replica_id):        

       jd = description()  
       jd.executable = self.working_directory + "async_agent_8/" + str(replica_id) + "/namd2"
       jd.number_of_processes = "4" 
       jd.spmd_variation = "single"
       jd.arguments = ["NPT.conf"] 
       jd.working_directory = self.working_directory + "async_agent_8/" + str(replica_id) + "/"
   ...
   ##################################################################################
   # Once Job description is set, submit jobs for each replica on respective machines
   ##################################################################################
   ...
   ################################################################################
   # Get the energy value from respective subjob output files
   ################################################################################
   ...
   def get_energy(self, replica_id):
       ...
       if(i< self.RPB):
           ssh = subprocess.Popen(['ssh', 'ssarip1@sierra.futuregrid.org', 'cat' , self.working_directory+ "async_agent_8/"+ str(replica_id) + "/stdout-" + str(replica_id) + ".txt"], stdout=subprocess.PIPE)          
           ...
           ssh = subprocess.Popen(['ssh', 'ssarip1@india.futuregrid.org', 'cat' , self.working_directory+ "async_agent_8/"+ str(replica_id) + "/stdout-" + str(replica_id) + ".txt"], stdout=subprocess.PIPE)          
           stdoutfile = ssh.stdout.readlines()
           for line in stdoutfile:
               items = line.split()
               if len(items) > 0:
           ...
           ssh = subprocess.Popen(['ssh', 'ssarip1@alamo.futuregrid.org', 'cat' , self.working_directory+ "async_agent_8/"+ str(replica_id) + "/stdout-" + str(replica_id) + ".txt"], stdout=subprocess.PIPE)          
           stdoutfile = ssh.stdout.readlines()
           for line in stdoutfile:
               items = line.split()
               if len(items) > 0:
           ...
           ssh = subprocess.Popen(['ssh', 'ssarip1@hotel.futuregrid.org', 'cat' , self.working_directory+ "async_agent_8/"+ str(replica_id) + "/stdout-" + str(replica_id) + ".txt"], stdout=subprocess.PIPE)          
           stdoutfile = ssh.stdout.readlines()
           for line in stdoutfile:
               items = line.split()
               if len(items) > 0:
   ...
   ########################################################################################################################
   # Swap the temperature values of replica-pair if they satisfy Metropolis Scheme, if not keep the same temperature values
   ########################################################################################################################
   ...
   ##########################################################################
   #  Stop Pilot Job
   #  Please change the value in range function depending on Big-jobs you use
   ##########################################################################
   ...
   def run_REMDg(self):
       ...
       ofilename = "async-remd-temp-4R-4BJ-2EX-out"
       #pdb.set_trace()
       replica_id= 0
                       ...
                       if (str(state) == "Done" and (iEX<numEX)):
                           ...
                           for frep in range(0,numReplica):
                               ...
                               if(str(state) == "Done" and (frep!=j)):
                                  ...
                                  iEX = iEX +1
                                  print "\n (Replica Exchange INFO): " + str(iEX)
                                  if(iEX < numEX): 

                                         #####################################################
                                         # Restart Replica irep
                                         #####################################################
                     		         if(irep < RPB):
                                            self.prepare_NAMD_config(irep,RESOURCEMGR_URL0[10:])
                                            self.transfer_NPT(irep,RESOURCEMGR_URL0[10:])
                                            jd = self.get_job_description(irep)
                                            new_job = self.submit_subjob(irep,jd)
                                            self.replica_jobs[irep] = new_job
                                  
	                                 elif((irep >=RPB) and (irep <2*RPB)):
                                            self.prepare_NAMD_config(irep,RESOURCEMGR_URL1[10:])
          	                            self.transfer_NPT(irep,RESOURCEMGR_URL1[10:])
	                                    jd = self.get_job_description(irep)
                                            new_job = self.submit_subjob(irep,jd)
               		                    self.replica_jobs[irep] = new_job
           
                                         elif((irep >=2*RPB) and (irep <3*RPB)):
                                            self.prepare_NAMD_config(irep,RESOURCEMGR_URL2[10:])
                              		    self.transfer_NPT(irep,RESOURCEMGR_URL2[10:])
                                            jd = self.get_job_description(irep)
                                            new_job = self.submit_subjob(irep,jd)
                                            self.replica_jobs[irep] = new_job

                                         elif(irep >=3*RPB):
                                            self.prepare_NAMD_config(irep,RESOURCEMGR_URL3[10:])
                                            self.transfer_NPT(irep,RESOURCEMGR_URL3[10:])
                                            jd = self.get_job_description(irep)
                                            new_job = self.submit_subjob(irep,jd)
                                            self.replica_jobs[irep] = new_job
                              
                                         #####################################################
                                         # Restart Replica frep
                                         #####################################################
                                         if(frep < RPB):
                                            self.prepare_NAMD_config(frep,RESOURCEMGR_URL0[10:])
                                            self.transfer_NPT(frep,RESOURCEMGR_URL0[10:])
                                            jd = self.get_job_description(frep)
                                            new_job = self.submit_subjob(frep,jd)
                                            self.replica_jobs[frep] = new_job
                                  
                                         elif((frep >=RPB) and (frep <2*RPB)):
                                            self.prepare_NAMD_config(frep,RESOURCEMGR_URL1[10:])
                                            self.transfer_NPT(frep,RESOURCEMGR_URL1[10:])
                                            jd = self.get_job_description(frep)
                                            new_job = self.submit_subjob(frep,jd)
                                            self.replica_jobs[frep] = new_job
               
                                         elif((frep >=2*RPB) and (frep <3*RPB)):
                                            self.prepare_NAMD_config(frep,RESOURCEMGR_URL2[10:])
                                            self.transfer_NPT(frep,RESOURCEMGR_URL2[10:])
                                            jd = self.get_job_description(frep)
                                            new_job = self.submit_subjob(frep,jd)
                                            self.replica_jobs[frep] = new_job

                                         elif(frep >=3*RPB):
                                            self.prepare_NAMD_config(frep,RESOURCEMGR_URL3[10:])
                                            self.transfer_NPT(frep,RESOURCEMGR_URL3[10:])
                                            jd = self.get_job_description(frep)
                                            new_job = self.submit_subjob(frep,jd)
                                            self.replica_jobs[frep] = new_job
                                  else:

...
class ReManager():
   ...
   def start_bigjob(self,COORDINATION_URL,RESMGR_URL,i):
       ...
       #####################################################
       try:
          #print "\n (Delete Remote Directory If Exists)" + "ssh "+ str(RESMGR_URL[10:])+ " rm -rf "+ self.working_directory+"async_agent"
          os.system(" ssh " + str(RESMGR_URL[10:]) + " rm -rf "+ self.working_directory + "async_agent")
          #print "\n (Success) Deleted Existing BigJobAgent Working Directory"
          os.system(" ssh " + str(RESMGR_URL[10:]) + " mkdir "+ self.working_directory + "async_agent")
   ...
   def stage_in_files(self,replica_id,RESMGR_URL):
       ...
       if(i<self.RPB):
          ...
          try:
             os.system(" ssh " + str(RESMGR_URL) + " mkdir "+ self.working_directory + "async_agent/"+ str(replica_id))
   ...
   def run_REMDg(self):
       ...
       for i in range (0, self.total_number_replica):
           if(i<RPB):
             ...
             replica_id = replica_id + 1
       end_time = time.time()
       print "\n (INFO) Total time to Stage files TFile is: " + str(end_time-start_time)
       ...
       while (iEX < numEX): 
            ...
            for irep in range(0, numReplica):
                       ...
                       try:
                           ...
                           pass
                       print "replica_id: " + str(irep) + " job: " + str(running_job) + "received_state: " + str(state) + " Time since launch: " + str(time.time()-REMD_Start) + " sec"
                       ...
                       if ((str(state) == "Done") and (iEX <numEX)):
                           ...
                           for frep in range(0,numReplica):
                               ...
                               if(str(state) == "Done" and (frep!=j)):
                                  ...
                                  print "\n (INFO) Waiting Time for replica_id: "+ str(j) +" is " + str(times[j])
                                  print "\n (INFO) Waiting Time for replica_id: "+ str(frep) +" is " + str(times[frep])

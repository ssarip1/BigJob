...
class ReManager():
   ...
   def start_bigjob(self,COORDINATION_URL,RESMGR_URL,i):
       ...
       workingdirectory= os.path.join(os.getcwd(), "sync_agent_4") # working directory for agent.
       ##########################################################################################
       #pdb.set_trace()
       #self.bjs=[]
       print "\n (VARIABLE) " + str(i)
   ...
   def stage_in_files(self,replica_id,RESMGR_URL):
       ...
       if(i<self.RPB):
          try:
             os.mkdir(self.working_directory + "sync_agent_4/" + str(replica_id))
             os.system("cp -r " + self.replica_directory + "* " + self.working_directory + "sync_agent_4/" + str(replica_id) + "/")
          ...
          try:
             ...
             os.system("scp -r " + self.replica_directory + "* " + str(RESMGR_URL)+ ":"+self.working_directory + "sync_agent_4/" + str(replica_id) + "/")
             #print "\n (INFO) total time taken to stage files on " + str(RESMGR_URL) + "is : "+ str(time.time()-start)
          except:
             print "\n (INFO) Error" + str(RESMGR_URL) 
   ...
   def transfer_NPT(self, replica_id,RESOURCEMGR_URL):
       ...
       #print "\n (INFO) " + "scp " + self.working_directory + "NPT.conf " + str(RESOURCEMGR_URL) + ":" +self.working_directory + "sync_agent_4/" + str(replica_id) + "/"
       try:    
          os.system("scp " + self.working_directory + "NPT.conf " + str(RESOURCEMGR_URL) + ":" +self.working_directory + "sync_agent_4/" + str(replica_id) + "/")
   ...
   def get_job_description(self, replica_id):        
       ...
       jd = description()  
       jd.executable = self.working_directory + "sync_agent_4/" + str(replica_id) + "/namd2"
       jd.number_of_processes = "4" 
       jd.spmd_variation = "single"
       jd.arguments = ["NPT.conf"] 
       jd.working_directory = self.working_directory + "sync_agent_4/" + str(replica_id) + "/"
   ...
   def get_energy(self, replica_id):
       ...
       if(i< self.RPB): 
           ssh = subprocess.Popen(['ssh', 'ssarip1@sierra.futuregrid.org', 'cat' , self.working_directory+ "sync_agent_4/"+ str(replica_id) + "/stdout-" + str(replica_id) + ".txt"], stdout=subprocess.PIPE)         
           ...
           ssh = subprocess.Popen(['ssh', 'ssarip1@india.futuregrid.org', 'cat' , self.working_directory+ "sync_agent_4/"+ str(replica_id) + "/stdout-" + str(replica_id) + ".txt"], stdout=subprocess.PIPE)          
           stdoutfile = ssh.stdout.readlines()
           for line in stdoutfile:
               items = line.split()
               if len(items) > 0:
           ...
           ssh = subprocess.Popen(['ssh', 'ssarip1@alamo.futuregrid.org', 'cat' , self.working_directory+ "sync_agent_4/"+ str(replica_id) + "/stdout-" + str(replica_id) + ".txt"], stdout=subprocess.PIPE)          
           stdoutfile = ssh.stdout.readlines()
           for line in stdoutfile:
               items = line.split()
               if len(items) > 0:
           ...
           ssh = subprocess.Popen(['ssh', 'ssarip1@hotel.futuregrid.org', 'cat' , self.working_directory+ "sync_agent_4/"+ str(replica_id) + "/stdout-" + str(replica_id) + ".txt"], stdout=subprocess.PIPE)          
           stdoutfile = ssh.stdout.readlines()
           for line in stdoutfile:
               items = line.split()
               if len(items) > 0:
